 ...
 * @author Wolverine
 */
public class FrmConsultaControleLadies extends javax.swing.JFrame implements  IControleLadies{
    
    private ConsultaControleLadiesTableModel modelVendas;
     ...
     */
    public FrmConsultaControleLadies(UsuarioLogadoBean usuarioLogadoBean) {
    ...
    private void initComponents() {
        ...
        nomeClientejTextField.setToolTipText("");
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addComponent(nomeClientejTextField, javax.swing.GroupLayout.PREFERRED_SIZE, 283, javax.swing.GroupLayout.PREFERRED_SIZE))
    ...
    private void AdicionarjButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AdicionarjButton3ActionPerformed
        if (usuarioLogadoBean.getAcesso().getPclatualizar()==1){
            int linha = vendasCursojTable.getSelectedRow();
            if (linha>=0){
                new FrmAtulizarControleLadies(listaVendas.get(linha), usuarioLogadoBean);
            }
    ...
    private void imprimirjButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_imprimirjButton1ActionPerformed
        new FrmFiltrarControleLadies(usuarioLogadoBean, this);
    ...
    private void imprimirjButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_imprimirjButton2ActionPerformed
        if (usuarioLogadoBean.getAcesso().getPclfinalizar()==1){
            int linha =vendasCursojTable.getSelectedRow();
            if (linha>=0){
                LadiesController ladiesController = new LadiesController();
                Controleladies controleladies =  ladiesController.consultarControleLadies(listaVendas.get(linha).getIdVendas());
                if (controleladies!=null){
                    controleladies.setFinalizado("Finalizado");
                    ladiesController.salvar(controleladies);
                }
                Ladies ladies = ladiesController.consultarLadies(listaVendas.get(linha).getIdVendas());
                if (ladies!=null){
                    ladies.setControle("Finalizado");
                    ladiesController.salvar(ladies);
                }
                carregarModelVendas();
            }
        }else JOptionPane.showMessageDialog(rootPane, "Acesso Negado");
    ...
    private void imprimirjButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_imprimirjButton5ActionPerformed
        ...
        if (linha>=0){
            LadiesController ladiesController = new LadiesController();
            Controleladies controle = ladiesController.consultarControleLadies(listaVendas.get(linha).getIdVendas());
            if (controle!=null){
    ...
    private void imprimirjButton6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_imprimirjButton6ActionPerformed
        if (usuarioLogadoBean.getAcesso().getInvoiceLancar() == 1) {
            ...
            if (linha >= 0) {
                ...
                new FrmCadastrarInvoiceController(listaVendas.get(linha).getIdVendas(), listaVendas.get(linha).getNomeCliente(), listaVendas.get(linha).getNomeFornecedor(), listaVendas.get(linha).getNomeFantasia(), "Ladies", parametrosprodutos.getLadies(), listaVendas.get(linha).getIdcontroleLadies());
            }
        }else JOptionPane.showMessageDialog(rootPane, "Acesso Negado");
...
public void carregarModelVendas(){
        ...
        if (usuarioLogadoBean.getUsuario().getTipo().equalsIgnoreCase("Gerencial")){
            sql = "Select v from Viewcontroleladies v where v.controle<>'Finalizado' and situacao<>'Cancelado'  and v.nomeCliente like '%" + nomeClientejTextField.getText() + "%' ";
        }else {
            sql = "Select v from Viewcontroleladies v where v.controle<>'Finalizado' and v.idUnidadeNegocio=" + usuarioLogadoBean.getUnidadeNegocio().getIdunidadeNegocio() + " and situacao<>'Cancelado' and v.nomeCliente like '%" + nomeClientejTextField.getText() + "%' ";
        }
        if (numero==0){
        ...
        LadiesController ladiesController = new LadiesController();
        listaVendas = ladiesController.consultaControleLadies(sql);
        if (listaVendas==null){
            listaVendas = new ArrayList<Viewcontroleladies>();
        }
        modelVendas = new ConsultaControleLadiesTableModel(listaVendas);
    ...
    public void setFiltrarControleLadies(Cliente cliente, Unidadenegocio unidadeNegocio, String paisDestino, String situacao, int idConsultor) {
        ...
        sql = "Select v from Viewcontroleladies v where ";
        if (cliente!=null){
        ...
        if (paisDestino.length()>0){
            if (condicao){
                sql = sql + " and v.pais='" + paisDestino + "' ";
            }else {
                sql = sql + " v.pais='" + paisDestino + "' ";
                condicao=true;
            }
    ...
    public void exportarExcel(String caminho, String nomeArquivo) {
        ...
        ladiesController.ExportarExcel(nomeArquivo, local, porta, senha, banco, usuario, caminho, idunidade);
    }
    ...
    public void setDocAnexado(String tipoArquivo, int idVenda, int idControle) {
        LadiesController ladiesController = new LadiesController();
        Controleladies controle;
        if (tipoArquivo.equalsIgnoreCase("Ficha Assinada")) {
             controle = ladiesController.consultarControleLadies(idVenda);
            if (controle != null) {
                ...
                ladiesController.salvar(controle);
            }
