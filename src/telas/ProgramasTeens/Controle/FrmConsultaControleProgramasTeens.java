 ...
 * @author Wolverine
 */
public class FrmConsultaControleProgramasTeens extends javax.swing.JFrame implements  IControleProgramasTeens{
    
    private ConsultaControleProgramasTeensTableModel modelVendas;
     ...
     */
    public FrmConsultaControleProgramasTeens(UsuarioLogadoBean usuarioLogadoBean) {
    ...
    private void initComponents() {
        ...
        setTitle("Controle de Programas Teens");
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addComponent(nomeClientejTextField, javax.swing.GroupLayout.PREFERRED_SIZE, 267, javax.swing.GroupLayout.PREFERRED_SIZE))
    ...
    private void AdicionarjButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AdicionarjButton3ActionPerformed
        if (usuarioLogadoBean.getAcesso().getPcctatualizar()==1){
            int linha = vendasCursojTable.getSelectedRow();
            if (linha>=0){
                new FrmAtulizarControleProgramasTeens(listaVendas.get(linha), usuarioLogadoBean);
            }
    ...
    private void imprimirjButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_imprimirjButton1ActionPerformed
        new FrmFiltrarControleProgramasTeens(usuarioLogadoBean, this);
    ...
    private void imprimirjButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_imprimirjButton2ActionPerformed
        if (usuarioLogadoBean.getAcesso().getPcctfinalizar()==1){
            int linha =vendasCursojTable.getSelectedRow();
            if (linha>=0){
                ProgramasTeensController programasTeensController = new ProgramasTeensController();
                Controleprogramasteens  controle =  programasTeensController.consultarControleProgramasTeens(listaVendas.get(linha).getIdVendas());
                if (controle!=null){
                    controle.setFinalizado("Finalizado");
                    programasTeensController.salvar(controle);
                }
                Programasteens programasteens = programasTeensController.consultarProgramasteens(listaVendas.get(linha).getIdVendas());
                if (programasteens!=null){
                    programasteens.setControle("Finalizado");
                    programasTeensController.salvar(programasteens);
                }
                carregarModelVendas();
            }
        }else JOptionPane.showMessageDialog(rootPane, "Acesso Negado");
    ...
    private void imprimirjButton6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_imprimirjButton6ActionPerformed
        if (usuarioLogadoBean.getAcesso().getInvoiceLancar() == 1) {
            ...
            if (linha >= 0) {
                ...
                new FrmCadastrarInvoiceController(listaVendas.get(linha).getIdVendas(), listaVendas.get(linha).getNomeCliente(), "Fornecedor", listaVendas.get(linha).getNomeFantasia(), "Programas Teens", parametrosprodutos.getProgramasTeens(), listaVendas.get(linha).getIdcontroleProgramasTeens());
            }
        }else JOptionPane.showMessageDialog(rootPane, "Acesso Negado");
...
public void carregarModelVendas(){
        ...
        if (usuarioLogadoBean.getUsuario().getTipo().equalsIgnoreCase("Gerencial")){
            sql = "Select v from Viewcontroleprogramasteens v where v.finalizado<>'Finalizado' and situacao<>'Cancelado' and v.nomeCliente like '%" + nomeClientejTextField.getText() + "%' ";
        }else {
            sql = "Select v from Viewcontroleprogramasteens v where v.finalizado<>'Finalizado' and v.idUnidadeNegocio=" + usuarioLogadoBean.getUnidadeNegocio().getIdunidadeNegocio() + " and situacao<>'Cancelado' and v.nomeCliente like '%" + nomeClientejTextField.getText() + "%' ";
        }
         if (numero==0){
        ...
        ProgramasTeensController programasTeensController = new ProgramasTeensController();
        listaVendas = programasTeensController.consultaControleProgramasTeens(sql);
        if (listaVendas==null){
            listaVendas = new ArrayList<Viewcontroleprogramasteens>();
        }
        modelVendas = new ConsultaControleProgramasTeensTableModel(listaVendas);
    ...
    public void setFiltrarControleProgramasTeens(Cliente cliente, Unidadenegocio unidadeNegocio, String paisDestino, String situacao) {
        ...
        sql = "Select v from Viewcontroleprogramasteens v where ";
        if (cliente!=null){
        ...
        if (paisDestino.length()>0){
            if (condicao){
                sql = sql + " and v.paisCurso='" + paisDestino + "' ";
            }else {
                sql = sql + " v.paisCurso='" + paisDestino + "' ";
                condicao=true;
            }
        }
    ...
    public void exportarExcel(String caminho, String nomeArquivo) {
        ...
        programasTeensController.ExportarExcel(nomeArquivo, local, porta, senha, banco, usuario, caminho, idunidade);
    }
    ...
    public void setDocAnexado(String tipoArquivo, int idVenda, int idControle) {
        ProgramasTeensController programasTeensController = new ProgramasTeensController();
        Controleprogramasteens controle;
        if (tipoArquivo.equalsIgnoreCase("Ficha Assinada")) {
             controle = programasTeensController.consultarControleProgramasTeens(idVenda);
            if (controle != null) {
